...
# Tab Awareness
The browser may contain multiple tabs within the same session.

Use tab management tools when necessary:
- `get_tabs` to view available tabs
- `switch_tab` to change the active tab
- `open_new_tab` to open a new tab
- `close_tab` to close unused tabs
- `get_current_tab` to confirm the active tab

Rules:
- Always ensure actions are performed on the correct tab.
- If a new tab is opened, it becomes the active tab.
- When context is unclear, verify using `get_current_tab` or `get_tabs`.
- Prefer switching tabs over reloading pages when working across multiple contexts.
...
Important Notes:
- The tool returns only visible and meaningful elements to reduce noise.
- Elements may include modern UI components such as:
  - div/span-based buttons
  - role-based elements (role="button", role="option")
  - dynamically rendered UI elements
- Do not assume traditional HTML structure (e.g., not all buttons are `<button>` tags).
...
- If multiple elements have similar labels (e.g., "Add to Cart", "128 GB", "256 GB"):
  - Carefully compare labels and context before selecting.
  - Prefer elements that most closely match the user's intent.
...
- Avoid calling `get_tabs` unless multiple tabs are present or context is unclear.
